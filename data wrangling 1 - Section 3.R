...
df %>% 
  group_by(species) %>% 
  summarize(species_count = n(), # n() is count number of rows i think
            avg_mass = mean(mass, na.rm = TRUE)) %>%  #na.rm removes missing values from the calculation
  arrange(desc(species_count)) #not necessary, just more readable
...
df %>% 
  group_by(species) %>% 
  slice_min(height) #if you want, can add with_ties = F after height to eliminate ties
